...
static int try_record_command_buffers(struct glfw_handler *this) {
	for (int i = 0; i < this->swapchain.image_count; ++i) {
		...
		render_pass_begin_info.renderPass = this->pipeline.render_pass;
		render_pass_begin_info.framebuffer = this->pipeline.framebuffers[i];
		render_pass_begin_info.renderArea.offset = render_area_offset;
		render_pass_begin_info.renderArea.extent = this->swapchain.extent;
		render_pass_begin_info.clearValueCount = 1;
		render_pass_begin_info.pClearValues = &clear_value;

		vkCmdBeginRenderPass(this->swapchain.command_buffers[i], &render_pass_begin_info, VK_SUBPASS_CONTENTS_INLINE);
		vkCmdBindPipeline(this->swapchain.command_buffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, this->pipeline.pipeline);
		vkCmdDraw(this->swapchain.command_buffers[i], 3, 1, 0, 0);
		vkCmdEndRenderPass(this->swapchain.command_buffers[i]);
...
static try_recreate_swapchain(struct glfw_handler *this) {
	...
	vk_pipeline__free_pipeline(&this->pipeline);
	vk_swapchain__free_swapchain(&this->swapchain);

	if (vk_swapchain__try_init_swapchain(&this->swapchain, width, height, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT) < 0) {
		return -1;
	}

	if (vk_pipeline__try_init_pipeline(&this->pipeline) < 0) {
	    return -2;
	}
...
int glfw_handler__try_init(struct glfw_handler *this, int width, int height, char *title, int is_fullscreen) {
	...
	result = vk_swapchain__try_init_swapchain(&this->swapchain, width, height, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
	if (result < 0) {
	...
	result = vk_pipeline__try_init(&this->pipeline, &this->swapchain);
	if (result < 0) {
		vk_swapchain__free_swapchain(&this->swapchain);
		vk_swapchain__free(&this->swapchain);
        vk_base__free(&this->base);
    ...
    result = vk_pipeline__try_init_pipeline(&this->pipeline);
    if (result < 0) {
		vk_pipeline__free(&this->pipeline);
		vk_swapchain__free_swapchain(&this->swapchain);
		vk_swapchain__free(&this->swapchain);
        vk_base__free(&this->base);
        free_glfw(this);
	...
	result = vk_staging_image__try_init(&this->test_texture, &this->base, width, height);
	if (result < 0) {
...
void glfw_handler__free(struct glfw_handler *this) {
	vk_staging_image__free(&this->test_texture);
    free_semaphores_and_fences(this);
	vk_pipeline__free_pipeline(&this->pipeline);
	vk_pipeline__free(&this->pipeline);
	vk_swapchain__free_swapchain(&this->swapchain);
